...
import sdk.kasookoo.ai.databinding.ActivityRegistrationBinding
                            ...
                            lifecycleScope.launch {
                                try {
                                    ...
                                    val userId = when (userData) {
                                        is sdk.kasookoo.ai.data.RandomUserResponse -> userData.id
                                        is sdk.kasookoo.ai.data.RandomCustomerLeadResponse -> userData.id
                                        else -> throw Exception("Invalid user data type")
                                    }
                                    
                                    val fullNameToSave = when (userData) {
                                        is sdk.kasookoo.ai.data.RandomUserResponse -> "${userData.first_name} ${userData.last_name}"
                                        is sdk.kasookoo.ai.data.RandomCustomerLeadResponse -> userData.full_name
                                        else -> "User"
                                    }
                                    
                                    val emailToSave = when (userData) {
                                        is sdk.kasookoo.ai.data.RandomUserResponse -> userData.email
                                        is sdk.kasookoo.ai.data.RandomCustomerLeadResponse -> userData.email
                                        else -> email
                                    }
                                    
                                    val phoneToSave = when (userData) {
                                        is sdk.kasookoo.ai.data.RandomUserResponse -> userData.phone_number
                                        is sdk.kasookoo.ai.data.RandomCustomerLeadResponse -> userData.phone_number
                                        else -> ""
                                    }
