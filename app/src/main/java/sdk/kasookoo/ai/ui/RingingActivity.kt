...
import sdk.kasookoo.ai.databinding.ActivityRingingBinding
import kotlinx.coroutines.launch
...
class RingingActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        runCatching {
            val userType = sdk.kasookoo.ai.data.UserDataManager(this).getUserType()
    ...
    private fun handleSilentNotificationCall() {
        try {
            ...
            when (callType) {
                "customer" -> {
                    sdk.kasookoo.ai.ui.MainActivity.setCurrentCallType(CallType.CUSTOMER)
                    Log.d(TAG, "🔕 Set call type to CUSTOMER")
                }
                "driver" -> {
                    sdk.kasookoo.ai.ui.MainActivity.setCurrentCallType(CallType.DRIVER)
                    Log.d(TAG, "🔕 Set call type to DRIVER")
                }
                "support" -> {
                    sdk.kasookoo.ai.ui.MainActivity.setCurrentCallType(CallType.SUPPORT)
            ...
            if (callType == "support") {
                sdk.kasookoo.ai.ui.MainActivity.setCurrentSupportCallParticipantIdentity(participantIdentity)
                sdk.kasookoo.ai.ui.MainActivity.setCurrentSupportCallRoomName(roomName)
                ...
                liveKitManager.callState.collect { state ->
                    ...
                    when (state) {
                        ...
                        CallState.IN_CALL -> {
                            ...
                            Log.d(TAG, "👤 Is customer: $isCustomer")
                            Log.d(TAG, "📞 Current call type: ${sdk.kasookoo.ai.ui.MainActivity.getCurrentCallType()}")
    ...
    private fun getTokenAndConnectToRoom(roomName: String) {
        ...
        tokenRequestScope.launch {
            try {
                val userDataManager = sdk.kasookoo.ai.data.UserDataManager(this@RingingActivity)
                val repository = sdk.kasookoo.ai.data.CallRepository()
                ...
                tokenResult.onSuccess { tokenResponse ->
                    ...
                    val callType = if (isCustomer) {
                        sdk.kasookoo.ai.core.CallType.CUSTOMER
                    } else {
                        sdk.kasookoo.ai.core.CallType.DRIVER
                    }

                    // Ensure global call type is set for downstream UI (e.g., CallActivity end flow)
                    sdk.kasookoo.ai.ui.MainActivity.setCurrentCallType(callType)
    ...
    private fun getLocalUserType(): String {
        val udm = sdk.kasookoo.ai.data.UserDataManager(this)
    ...
    private fun endCall() {
        ...
        // If this is an incoming call (callee side) and user is rejecting, notify backend
        try {
            if (isIncomingCall) {
                val udm = sdk.kasookoo.ai.data.UserDataManager(this)
                val repository = sdk.kasookoo.ai.data.CallRepository()
                val roomName = intent.getStringExtra("room_name") ?: silentRoomName ?: ""
                val participantIdentity = intent.getStringExtra("participant_identity") ?: silentParticipantIdentity ?: ""
                val participantIdentityName = udm.getFullName() ?: "User"
                val participantIdentityType = udm.getUserType() ?: if (isCustomer) "customer" else "driver"
                ...
                if (roomName.isNotEmpty() && calledUserId.isNotEmpty()) {
                    kotlinx.coroutines.CoroutineScope(kotlinx.coroutines.Dispatchers.IO).launch {
                        ...
                        result.onSuccess { resp ->
                            Log.d(TAG, "✅ Sent rejectIncomingCall from RingingActivity: ${resp.message}")
                        }.onFailure { e ->
                            Log.e(TAG, "❌ rejectIncomingCall failed: ${e.message}")
                        }
                    }
                }
            }
        } catch (e: Exception) {
            Log.e(TAG, "❌ Error sending rejectIncomingCall: ${e.message}")
        }
